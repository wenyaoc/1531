...
|channel/MESSAGES_DATA|GET|(token, channel_id, start)|{ MESSAGES_DATA, start, end }|**InputError** when any of:<ul><li>Channel ID is not a valid channel</li><li>start is greater than the total number of MESSAGES_DATA in the channel</li></ul>**AccessError** when<ul><li>Authorised user is not a member of channel with channel_id</li></ul>|Given a Channel with ID channel_id that the authorised user is part of, return up to 50 MESSAGES_DATA between index "start" and "start + 50". Message with index 0 is the most recent message in the channel. This function returns a new index "end" which is the value of "start + 50", or, if this function has returned the least recent MESSAGES_DATA in the channel, returns -1 in "end" to indicate there are no more MESSAGES_DATA to load after this return.|
...
|search|GET|(token, query_str)|{ MESSAGES_DATA }|N/A|Given a query string, return a collection of MESSAGES_DATA in all of the channels that the user has joined that match the query|
